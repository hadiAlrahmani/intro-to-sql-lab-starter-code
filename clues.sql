...
FROM cities
WHERE name LIKE '%San%'
AND countrycode IN (SELECT code FROM countries WHERE continent = 'South America');
...
SELECT name
FROM cities
WHERE countrycode = (SELECT code FROM countries WHERE name = 'Argentina')
AND id = (SELECT capital FROM countries WHERE name = 'Argentina');
...
-- population excluding carmen.

-- closest one is Santa Monica, with a population of 91,084.
